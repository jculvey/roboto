...
Crawler.prototype.on('finish', function() {
  ...
  log.info('Finished crawled.');
  log.info('  >> Started: %s ', this.stats.startTime.format(prettyFormat));
  log.info('  >> Finished: %s ', now.format(prettyFormat));
  log.info('  >> Duration: %s hours, %s minutes, %s seconds.', hours, mins, secs);
  log.info('  >> Page(s) crawled: %s', this.stats.pagesCrawled);
  log.info('  >> Page(s) nofollowed: %s', this.stats.nofollowed);
  log.info('  >> Request count: %s', this.stats.download.requestCount);
  log.info('  >> Request errors: %s', this.stats.download.requestErrors);
  log.info('  >> 200 OK count : %s', this.stats.download.status200);
  log.info('  >> 301 count : %s', this.stats.download.status301);
  log.info('  >> 302 count : %s', this.stats.download.status302);
  log.info('  >> 404 count : %s', this.stats.download.status404);
  log.info('  >> Filtered due to');
  log.info('    >> Disallowed domain: %s', this.stats.download.disallowedDomain);
  log.info('    >> Blacklist: %s', this.stats.download.blacklisted);
});
