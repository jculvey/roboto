...
var Crawler = function(options) {
  ...
  botParser = new BotParser({ userAgent: this.options.userAgent });
...
Crawler.prototype._addUrl= function(url) {
  if(!_.contains(this._seen, url)) {
    this._queue.push(url);
    this._seen.push(url);
  }
  else {
    this.log.debug('Filtering previously seen url: %s', url);
  }
};


Crawler.prototype._crawlNext = function(done) {
  var nextUrl = this._getNextUrl();
  this.emit('next', nextUrl);

  setTimeout(this._crawlPage.bind(this),
...
Crawler.prototype._crawlPage = function(url, done) {
  ...
  botParser.allowed(url, function(allowed) {
    if (allowed) {
